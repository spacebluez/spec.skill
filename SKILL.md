---
name: mew-spec
description: "版本化 Spec 驱动开发：在 mew/Tn 下用 chat.md 记录需求澄清，并依次生成 spec.md → plan.md → task.md → checklist.md，然后指导开发和验收。在开始任何功能、模块或章节开发前使用。"
---

# Spec 驱动开发

把想法变成可运行的代码，中间经过一份会话记录和四份递进文档：
...
`chat.md` 只记录需求澄清阶段，并在生成 `spec.md` 前停止追加；其余每份文档都在前一份基础上细化，并且需要用户审批后才能进入下一阶段。

> [!IMPORTANT]
> <HARD-GATE> **HARD GATE：**四份文档全部生成并获得用户批准之前，禁止编写任何实现代码。无论项目看起来多简单，一律走完流程。 </HARD-GATE>
...
| 文档 | 回答什么 | 包含什么 |
| --- | --- | --- |
| `chat.md` | 需求澄清阶段讨论了什么 | 初始提示词、澄清问题、完整选项、用户选择、方案比较和澄清结论 |
...
每次启动本 Skill，必须先确定本次唯一可写的版本目录，再进行需求澄清或生成文档。

1. 检查项目根目录下的 `mew/`，只将名称严格匹配 `T数字` 的目录视为版本目录。
2. 按数字比较版本号（例如 `T10` 晚于 `T9`），找出版本号最大的目录。
3. 如果不存在版本目录，创建并使用 `mew/T1/`。
4. 如果最新版目录完全为空（其中没有任何文件或子目录），直接使用该目录。
5. 如果最新版目录中存在任何文件或子目录，创建并使用下一版 `mew/T{最大版本号+1}/`。
6. 若准备创建的下一版目录已存在，重新扫描并确定版本，不得覆盖或复用有内容的目录。
7. 本次生成的 `chat.md`、`spec.md`、`plan.md`、`task.md`、`checklist.md` 必须全部位于本次选定的同一个版本目录。
8. 除本次选定目录外，所有既有版本目录均为只读；禁止覆盖、追加、修改、移动或删除其中的任何内容。
9. 后文出现的文档名均指本次版本目录内的对应文件，不是项目根目录下的同名文件。
...
└────────────────────────────┬─────────────────────────────┘
                             ▼
┌──────────────────────────────────────────────────────────┐
│ 阶段零：确定版本目录并建立 chat.md                       │
│          chat.md 仅在需求澄清阶段持续记录                │
└────────────────────────────┬─────────────────────────────┘
                             ▼
┌──────────────────────────────────────────────────────────┐
│ 阶段一：需求澄清 → spec.md → 用户审批                    │
│          未通过：修改 spec.md 后重新审批                 │
└────────────────────────────┬─────────────────────────────┘
                             ▼ 通过
┌──────────────────────────────────────────────────────────┐
│ 阶段二：技术设计 → plan.md → 用户审批                    │
│          未通过：修改 plan.md 后重新审批                 │
└────────────────────────────┬─────────────────────────────┘
                             ▼ 通过
┌──────────────────────────────────────────────────────────┐
│ 阶段三：任务拆解 → task.md → 用户审批                    │
│          未通过：修改 task.md 后重新审批                 │
└────────────────────────────┬─────────────────────────────┘
                             ▼ 通过
┌──────────────────────────────────────────────────────────┐
│ 阶段四：验收设计 → checklist.md → 用户审批               │
│          未通过：修改 checklist.md 后重新审批            │
└────────────────────────────┬─────────────────────────────┘
                             ▼ 通过
┌──────────────────────────────────────────────────────────┐
│ 阶段五：开发（按 task.md 执行）                          │
...
---
阶段零：需求澄清记录 → chat.md
输入： 用户启动本次 mew-spec 时给出的原始提示词，以及需求澄清阶段中用户与助手的全部相关讨论
输出： 本次版本目录下的 chat.md
流程
1. 确定本次版本目录后，立即创建 chat.md。
2. 将用户启动本次流程时给出的提示词作为第一条消息记录。
3. 在需求澄清阶段，每次用户或助手发表相关内容时，按实际发生顺序追加记录。
4. 保留澄清问题、方案比较、修改意见、被否决的方案和最终澄清结论，不能只记录最终结论。
5. 助手提出选择题时，完整记录问题和全部选项，不得只记录选项编号或推荐结论。
6. 用户用 `1`、`2`、`3`、字母或其他简短形式作答时，完整保留此前助手给出的全部选项，并明确记录用户选择的编号和对应选项内容。
7. 需求澄清完成后、开始生成 spec.md 前，执行自检并停止向 chat.md 追加内容。spec 审批及后续设计、任务拆解、验收设计、开发和验收阶段的对话一律不记录。
chat.md 模板
# [标题] Chat

> 本文档仅记录本版本的初始提示词和需求澄清阶段讨论。
...
[助手提出的澄清问题；如为选择题，完整记录问题和全部选项]

### 用户

[用户回复；如以编号等简短形式选择，同时写明“选择 N：[对应选项完整内容]”]

### 助手

[助手后续澄清内容]
写作规则
- 按时间顺序记录。 消息顺序必须与实际讨论顺序一致。
- 区分发言者。 每条消息明确标注“用户”或“助手”。
- 保留原意。 原则上原样记录，不把对话改写成摘要。
- 记录范围严格限定。 只包含初始提示词和需求澄清阶段的讨论；不记录 spec 审批及后续阶段内容。
- 完整记录选项。 助手提出选择题时，问题、全部选项、各选项说明和推荐理由都必须保留。
- 补全简短选择。 用户仅回复编号、字母等标识时，在该条用户记录中同时注明标识及其对应选项完整内容；不得让记录脱离上文后无法理解。
- 只记录本次迭代。 不得混入其他版本的对话。
- 到点停止。 需求澄清完成后、生成 spec.md 前停止追加，chat.md 不需要单独审批。
- 不得修改历史记录。 停止记录后，不得因后续审批、开发或验收讨论而修改该版本的 chat.md。
- 不记录系统内部内容。 不记录系统提示、工具调用细节、隐藏推理或与本次工作无关的内容。
- 保护敏感信息。 密码、令牌、密钥等敏感信息使用 `[已隐藏敏感信息]` 替代，不得原样写入。
自检
1. 起始提示检查——是否记录了用户启动本次流程时的完整提示词？
2. 顺序检查——用户和助手的消息是否按照实际发生顺序排列？
3. 澄清完整性检查——重要的澄清、方案比较、修改意见、否决方案和澄清结论是否都有记录？
4. 选项完整性检查——每道选择题是否完整保留了问题、全部选项、说明和推荐理由？
5. 选择可读性检查——用户仅回复编号或字母时，是否同时写明了对应选项的完整内容？
6. 范围检查——是否误记了 spec 审批或技术设计、任务拆解、验收设计、开发、验收阶段的内容？
7. 版本检查——内容是否全部属于当前版本？
8. 历史保护检查——是否只写入当前版本，没有修改以前版本的 chat.md？
9. 敏感信息检查——是否误写入密码、令牌或密钥？
---
阶段一：需求澄清 → spec.md
输入： 用户的初步想法或粗略描述
输出： spec.md
...
发现问题就地修正，然后进入用户审批。
用户审批
> spec.md 已生成。请 review： > - 功能需求是否完整？ > - 有没有遗漏的边界情况？ > - 「不做的事」是否合理？ > - 验收标准是否可观测？ > > 确认后进入技术设计阶段。
...
## 文件组织
project/ ├── internal/prompt/ │   ├── builder.go    — Builder、Section 类型、BuildSystemPrompt │   ├── sections.go   — 8 个固定 section 函数 │   └── plan_mode.go  — Plan Mode 提醒构造 └── ...
## 技术决策
...
用户审批
> plan.md 已生成。请 review： > - 架构划分是否合理？ > - 核心接口定义是否完整？ > - 模块间交互是否清晰？ > - 技术决策是否认同？ > > 确认后进入任务拆解阶段。
---
阶段三：任务拆解 → task.md
...
用户审批
> task.md 已生成，共 N 个任务。请 review： > - 任务粒度是否合适？ > - 依赖关系是否正确？ > - 有没有遗漏的实现步骤？ > > 确认后进入验收设计阶段。
---
...
用户审批
> checklist.md 已生成。请 review： > - 是否完整覆盖了 spec 的验收标准？ > - 每项是否都可以运行/观察验证？ > - 端到端场景是否合理？ > > 确认后进入开发阶段。
---
阶段五：开发
四份文档全部通过审批，开始实现。
